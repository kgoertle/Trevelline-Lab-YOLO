...
def run_detection(model, source, source_type, test_detect=False, smoother=None, src_idx=0, total_sources=1):
    ...
    DISPLAY_BASE_WIDTH, DISPLAY_BASE_HEIGHT = 1024, 768
    ...
    frame_width, frame_height = (640, 480) if source_type=="picamera" else (1024, 768)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out_writer = cv2.VideoWriter(str(out_file), fourcc, 30, (frame_width, frame_height))
    ...
    while True:
        ...
        if window_name is not None:
            ...
            if cv2.waitKey(1) & 0xFF == 27:
                esc_pressed = True
                print(f"[ESC] Closing {source_name}...")
