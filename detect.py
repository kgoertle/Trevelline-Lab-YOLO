...
def get_output_folder(weights_path, source_type, source_name, test_detect=False):
    ...
    out_dir = out_base / (source_name if source_type in ["usb", "picamera"] else "video-in")
    out_dir.mkdir(parents=True, exist_ok=True)
    return out_dir

class BoxSmoother:
    def __init__(self, max_history=0, alpha=1.0, dist_thresh=None):
        self.max_history = max_history; self.alpha = alpha; self.dist_thresh = dist_thresh; self.history = []

    def smooth(self, boxes):
        smoothed, new_history = [], []
        for box in boxes:
            x1, y1, x2, y2, cls = box; matched = None
            ...
            if matched:
                ...
                x1 = int(self.alpha*x1 + (1-self.alpha)*hx1); y1 = int(self.alpha*y1 + (1-self.alpha)*hy1)
                x2 = int(self.alpha*x2 + (1-self.alpha)*hx2); y2 = int(self.alpha*y2 + (1-self.alpha)*hy2)
...
def run_detection(model, source, source_type, test_detect=False, smoother=None, src_idx=0, total_sources=1):
    ...
    # --- Window setup (small display, auto-tiling) ---
    ...
    interaction_counter, overlap_history = 0, {}
    if smoother is None:
    ...
    while True:
        ...
        for bird in M_or_F:
            ...
            for obj in smoothed_boxes_list:
                ...
                if bname==oname: continue
                if oname in targets or (bname in ["M","F"] and oname in ["M","F"]):
                    ix1,iy1,ix2,iy2=max(bx1,ox1),max(by1,oy1),min(bx2,ox2),min(by2,oy2)
                    if ix2>ix1 and iy2>iy1:
                        key=(tuple(bird),tuple(obj))
                        new_overlap[key]=overlap_history.get(key,0)+1
                        if new_overlap[key]==(5 if test_detect else 60): interaction_counter+=1
        overlap_history = new_overlap
        ...
        if cv2.waitKey(1)&0xFF==27:
            esc_pressed=True; print(f"[INFO] ESC pressed, closing {source_name}...")
    ...
    print(f"[INFO] Detection results saved to: {out_file}")
